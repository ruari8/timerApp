...
import { playSound, playComplete, playTick, initAudio } from '../utils/audio';
...
export function TimerScreen({ navigation, route }: TimerScreenProps) {
  ...
  // Skip ahead to near end of current segment
  const handleSkip = useCallback(() => {
    ...
    setState((prev) => {
      ...
      return {
        ...prev,
        remainingSeconds: skipTo,
      };
    });
  }, [settings.skipAheadSeconds, isComplete]);

  const tick = useCallback(() => {
    setState((prev) => {
      if (prev.remainingSeconds <= 1) {
        // Get current segment to play its specific end sound
  ...
  }, [pattern, settings.defaultEndSound, settings.hapticFeedback]);
  ...
  return (
    <View style={[styles.container, { backgroundColor: segmentColor }]}>
      ...
      <View style={styles.controls}>
        ...
        <TouchableOpacity 
          style={[styles.skipBtn, isComplete && styles.skipBtnDisabled]} 
          onPress={handleSkip}
          disabled={isComplete}
        >
